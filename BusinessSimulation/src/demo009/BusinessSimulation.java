...
import demo009.menu.impl.CommandKaufFuhrwerke;
...
import demo009.menu.impl.CommandVerkaufFuhrwerke;
import demo009.menu.impl.CommandVerkaufen;
import demo009.menu.impl.CommandLesen;
...
public class BusinessSimulation {
	...
	private void initStartzustand() {
		...
		appContext.setFuhrpark(new Fuhrpark(1, 24));
		
	}
	
	private Menu initMainMenu() {
		
		// Untermenu für die Verwaltung der Arbeiter
		Submenu subArbeitende = new Submenu("Arbeiter verwalten", appContext, "Zurück");
		subArbeitende.setCommand(new CommandEinstellen());
		subArbeitende.setCommand(new CommandEntlassen());
		...
		subNiederlassung.setCommand(subArbeitende);
		...
		subFuhrpark.setCommand(new CommandKaufFuhrwerke());
		subFuhrpark.setCommand(new CommandVerkaufFuhrwerke() );
		...
		mainMenu.setCommand(new CommandLesen());
